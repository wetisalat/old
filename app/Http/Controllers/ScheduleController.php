...
class ScheduleController extends Controller
{
    ...
    public function schedule(Request $request)
    {
        if (request()->method() == 'GET')
        {
            ...
            $staff_users = DB::table('users')
                ->leftJoin('teams', 'teams.id', '=', 'users.current_team_id')
                ->leftJoin('staff', 'staff.user_id', '=', 'users.id')
                ->where('users.role_id', 2)
                ->where('users.user_id', auth()->user()->id)
                ->where('staff.is_contact', '=', 1)
            ...
            $leaveCategories = DB::table('leave_category')
                ->get()
                ->map(function($category) {
                    $category->leave_reasons = DB::table('leave_reasons')->where('leave_category', '=', $category->id)->get();
            ...
            if (Auth::User()->role_id == 1){
                return view('users/schedule/schedule', ['pageConfigs' => $pageConfigs, 'resource' => $resource, 'staff_users' => $staff_users, 'shifts' => $shifts, 'locations' => $locations, 'events' => $events, 'tasklist' => $getTaskData, 'lunch' => $lunch, 'leave_categories' => $leaveCategories, 'leave' => $leave]);
              }
              else{
        ...
        if($request->has('recurring')) {
            $data['recurring'] = json_encode($request->recurring);
        }
    ...
    public function getUserSchedule()
    {
        ...
        $events = $events->map(function($event) {
            ...
            if (!is_null($event->recurring)) {
                $newEvent->start = Carbon::parse($event->start)->format('H:i:s');
                $newEvent->end = Carbon::parse($event->end)->format('H:i:s');
                $newEvent->recurring = json_decode($event->recurring);
            }
