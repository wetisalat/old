...
class StaffController extends Controller
{
        ...
        if ($request['option'] == 0) {
            ...
            if ($staff) {
                DB::table('staff')->where('user_id', $request['id'])->update([
                    ...
                    'document_type' => $request['info-document_type'],
                    'expiry_date' => $request['info-expiry_date'],
                    'identification_number' => $request['info-identification_number']
                ]);
            } else {
    ...
    public function suspen_account(Request $request)
    {
        ...
        if(is_array($request_id)) {
            foreach ($request_id as $user_id) {
                
                $staff = DB::select("SELECT * FROM staff WHERE user_id = '" . $user_id . "'");

                if ($staff) {
                    if ($staff[0]->is_contact == 0) {
                        DB::table('staff')->where('user_id', $user_id)->update(['is_contact' => 1, 'updated_at' => $activeTime]);
                    } else {
                        DB::table('staff')->where('user_id', $user_id)->update(['is_contact' => 0]);
                    }
                } else {
                    DB::table('staff')->insert(['user_id' => $user_id, 'is_contact' => 1, 'updated_at' => $activeTime]);
                }
            }
        } else {
            $staff = DB::select("SELECT * FROM staff WHERE user_id = '" . $request_id . "'");
    ...
    public function view_salary($id)
    {
        ...
        $staff = DB::table('staff')
                ...
                ->where('staff.user_id', '=', $id)
                ->select('staff.*', 'contract_type.name as contract_name', 'location.name as location_name', 'location.bonus', 'staff.bonus_id as staff_bonus', 'night_amount.amount as night_amount', 'transport_amount.amount as transport_amount')
                ->get();
        ...
        return view('/users/staff/view-salary', [
            ...
            'locations' => $locations,
            'shiftDa' => $shiftAtt,
            'nightAmounts' => DB::table('night_amount')->where('user_id', '=', auth()->user()->id)->get(),
            'transportAmounts' => DB::table('transport_amount')->where('user_id', '=', auth()->user()->id)->get(),
        ]);
    }
        ...
        if ($request['option'] == 0) {
            if ($staff) {
                DB::table('staff')->where('user_id', $request['id'])->update([
                    ...
                    'time_type' => $request['wage-time_type'],
                    'hiring_date' => $request['wage-hiring_date'],
                    'end_date' => $request['wage-end_date'],
                    'contract_status' => $request['wage-contract_status'],
                    'categorie_professionnelle' => $request['wage-categorie_professionnelle'],
                    'echelon' => $request['wage-echelon'],
            ...
            if ($staff) {
                DB::table('staff')->where('user_id', $request['id'])->update([
                    ...
                    'bonus_id' => implode(',', $request->bonus),
                    'night_amount_id' => $request->night_amount,
                    'transport_amount_id' => $request->transport_amount
                ]);
            } else {
                DB::table('staff')->insert([
                    'monthly_salary' => $request['salary-monthly_salary'],
