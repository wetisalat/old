...
$(function () {
  ...
  var dt_project_table = $('.datatable-project'),
    ...
    addNewInfo = document.querySelector('.add-user-information'),
    updatePlan = document.querySelector('.user-plan-update'),
...
// Add user information button
    if (addNewInfo) {
        addNewInfo.onclick = function () {

            var userId = $("#userId").val();
            var address_form = $("#address_form").val();
            var firstname = $("#modalAddressFirstName").val();
            var lastname = $("#modalAddressLastName").val();
            var email = $("#modalAddressEmail").val();
            var phone = $("#modalAddressPhone").val();
            var n_company = $("#modalAddressNumberCompany").val();
            var vat_number = $("#modalAddressVAT").val();
            var address = $("#modalAddressAddress1").val();
            var city = $("#modalAddressTown").val();
            var postcode = $("#modalAddressZipCode").val();
            var country = $("#modalAddressCountry").val();
                ...
                success:function(response){
                    ...
                    if(response.message === 'success') {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success!',
                            text: 'Account Address Updated Successfully.',
                            customClass: {
...
// Add user Plan Upgrade
    if (updatePlan) {
        updatePlan.onclick = function () {

            var userId = $("#userId").val();
            var update_plan = $("#update_plan").val();
                ...
                success:function(response){
                    ...
                    if(response.message === 'success') {
                        Swal.fire({
                            icon: 'success',
                            title: 'Success!',
                            text: 'User Plan Updated Successfully.',
                            customClass: {
                                confirmButton: 'btn btn-success'
                            }
                        }).then(function (result) {
    ...
    if (changePassword) {
        changePassword.onclick = function () {
            ...
            function printErrorMsg (msg) {
                ...
                if (msg.password){
                    $(".password_errormessage").text(msg.password[0]);
                }
                if (msg.password_confirmation){
                    $(".confirm_errormessage").text(msg.password_confirmation[0]);
                }
