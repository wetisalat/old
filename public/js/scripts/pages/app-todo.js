    ...
    success:function(response){
      if(response.status == 1) {
        ...
        if(response.error.title_errormessage) {
          ...
          $('.new-todo-item-title').addClass('todoborder');
        }else {
          $('.new-todo-item-title').removeClass('todoborder');
        }

        if(response.error.assignee_errormessage) {
          $(".assignee_errormessage").text(response.error.assignee_errormessage);
          $('.select2').addClass('todoborder');
        }else{
          $('.select2').removeClass('todoborder');
        }

        if(response.error.due_date_errormessage) {
          $(".due_date_errormessage").text(response.error.due_date_errormessage);
          $('.task-due-date').addClass('todoborder');
        }else {
          $('.task-due-date').removeClass('todoborder');
        }

        if(response.error.tag_errormessage) {
          $(".tag_errormessage").text(response.error.tag_errormessage);
          $('.task-tagSelect .select2-container').addClass('todoborder');
        }else {
          $('.task-tagSelect .select2-container').removeClass('todoborder');
        }

        if(response.error.description_errormessage) {
          $(".description_errormessage").text(response.error.description_errormessage);
          $('#task-desc').addClass('todoborder');
        }else {
          $('#task-desc').removeClass('todoborder');
        }
      }
    }
  });
