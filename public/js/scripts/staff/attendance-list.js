/*=========================================================================================
    File Name: app-attendance-list.js
    Description: app-attendance-list Javascripts
    ----------------------------------------------------------------------------------------
...
$(function () {
  ...
  var dtattendanceTable = $('.attendance-list-table'),
    assetPath = '../../../app-assets/',
    attendancePreview = 'app-attendance-preview.html',
    attendanceAdd = 'app-attendance-add.html',
    attendanceEdit = 'app-attendance-edit.html';

  if ($('body').attr('data-framework') === 'laravel') {
    assetPath = $('body').attr('data-asset-path');
    attendancePreview = assetPath + 'app/attendance/preview';
    attendanceAdd = assetPath + 'app/attendance/add';
    attendanceEdit = assetPath + 'app/attendance/edit';
  }

  // datatable
  if (dtattendanceTable.length) {
    var dtattendance = dtattendanceTable.DataTable({
      ajax: assetPath + 'data/attendance-list.json', // JSON file to add data
      autoWidth: false,
      columns: [
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            var $attendanceId = full['attendance_id'];
            // Creates full output for row
            var $rowOutput = '<a class="fw-bold" href="' + attendancePreview + '"> #' + $attendanceId + '</a>';
            return $rowOutput;
          }
        },
        {
          ...
          width: '42px',
          render: function (data, type, full, meta) {
            var $attendanceStatus = full['attendance_status'],
            ...
            return (
              ...
              roleObj[$attendanceStatus].class +
              '">' +
              '<span class="avatar-content">' +
              feather.icons[roleObj[$attendanceStatus].icon].toSvg({ class: 'avatar-icon' }) +
              '</span>' +
              '</div>' +
              '</span>'
            );
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              '<a class="me-25" href="' +
              attendancePreview +
              '" data-bs-toggle="tooltip" data-bs-placement="top" title="Preview attendance">' +
      ...
      dom:
        ...
        '<"col-lg-6 d-flex align-items-center justify-content-lg-end flex-lg-nowrap flex-wrap pe-lg-1 p-0"f<"attendance_status ms-sm-2">>' +
        '>t' +
