                    ...
                    <div style="height:100%">
                        <div class="mbsc-grid mbsc-no-padding">
                          <div class="mbsc-row">
                              <div class="mbsc-col-sm-9 total-drop-calendar">
                                 	<div id="custom-event-attendance-popup" class="md-attendance">
                                      <div id="attendance-event-header" class="md-attendance-header">
                                          ...
                                          <div id="attendance-event-time" class="md-attendance-time" style="font-weight: 700;color: #fff"></div>
                                      </div>
                                      <div class="md-attendance-info">
                                          <div id="attendance-event-pause" class="md-attendance-title">Pause: <span class="md-attendance-pause md-attendance-text"></span></div>
                                          <div id="attendance-event-transport" class="md-attendance-title">Transport: <span class="md-attendance-transport md-attendance-text"></span></div>
                              ...
                              <div class="mbsc-col-sm-3" id="attendance-summary">
...
<script>
    ...
    $(function () {
        ...
        var $tooltipLocation = $('.md-attendance-location');
        var $tooltipExtra = $('.md-attendance-extra');
        var $tooltipComment = $('.md-attendance-comment');
        var $tooltipPause = $('.md-attendance-pause');
        var $tooltipTransport = $('.md-attendance-transport');
        ...
        onEventDoubleClick: function (event, inst) {
	     ...
	     resources: myResources,
            ...
            renderLabel: function (data) {
            	if (data.isMultiDay) {
		            ...
		            return '<div class="single-day-event-dot" style="color:' + data.original.color + '"></div>' +
		                '<div class="single-day-event" style="border-radius: 4px;text-align:center;color:#fff;font-weight:700;background: '+data.original.color +'">' + data.start + ' - ' + data.end + '</div>';
		        }
		    },
            ...
            onPageChange: function(event, inst) {
                getAttendanceSummary( formatDate('DD-MM-YYYY HH:mm:ss', event.month) );
            },
            ...
            onEventHoverIn: function (args, inst) {  // More info about onEventHoverIn: https://docs.mobiscroll.com/5-16-0/eventcalendar#event-onEventHoverIn
                ...
                var time = formatDate('HH:mm', new Date(event.start)) + ' - ' + formatDate('HH:mm', new Date(event.end));
                var button = {};
                ...
                if(event.gps_in) {
                    $tooltipLocation.text(event.gps_in);
                } else {
                    $tooltipLocation.text(event.location);
                }
                
                if(event.pause) {
                    $tooltipPause.text(event.pause);
                }

                if(event.transport) {
                    $tooltipTransport.text(event.transport);
                }

                if(event.extraLabel) {
                    var extraLabelHtml = '';

                    if(event.extraLabel.indexOf('Transport') !== -1) {
                        extraLabelHtml += feather.icons['truck'].toSvg({ class: 'ficon mx-1' });
                    }

                    if(event.extraLabel.indexOf('Nuitées') !== -1) {
                        extraLabelHtml += feather.icons['moon'].toSvg({ class: 'ficon mx-1' });
                    }

                    $tooltipExtra.html(extraLabelHtml);
                }
        ...
        function createEditPopup(args) {
                    ...
                    'cancel',
                    {
                        ...
                        handler: function () {
                            ...
                            calendar.updateEvent({
                                ...
                                comment: tempAttendance.comment,
                                start: date[0],
            ...
            $locationField.mobiscroll('getInst').value = tempAttendance.gps_in ? tempAttendance.gps_in : tempAttendance.location;
        ...
        customDate = $('#custom-repeat-date').mobiscroll().datepicker({
            ...
            display: 'anchored',
            touchUi: false,
            dateFormat: 'DD-MM-YYYY',                      // More info about dateFormat: https://docs.mobiscroll.com/5-16-0/eventcalendar#localization-dateFormat
        ...
        $comment.change(function(e) {
            tempAttendance.comment = $(this).val();
        });

        $locationField.change(function(e) {
            tempAttendance.location = $(this).val();
        });
        ...
        function saveAttendance() {
            ...
            tempAttendanceData.start = formatDate('DD-MM-YYYY HH:mm:ss', tempAttendance.start);
            tempAttendanceData.end = formatDate('DD-MM-YYYY HH:mm:ss', tempAttendance.end);
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    getAttendanceList(selectedFilters);
                    getAttendanceSummary(tempAttendanceData.start)
                }
            });
        }
        ...
        function getAttendanceSummary(date) {
            $.ajax({
                url: '{{ url("staff/attendance-summary") }}',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                type: 'POST',
                ...
                success: function(res) {
                   // populate weeks data
                   var rows = '';
                   var rowColors = ['#bac70c', '#34c8e0', '#f7961e', '#df0606', '#1064b0'];

                   var i = 0;

                   $.each(res.data.weeks, function(weekNumber, v) {
                        rows += '<div class="total-drop-task" style="background: '+rowColors[i]+';">'+
                            '<div class="week-title">Week '+weekNumber+'</div>'+
                            '<div class="text-center" style="display:flex; justify-content: space-between;">'+
                                '<div>Worked<br/><strong>'+v.worked+'</strong></div>'+
                                '<div>Overtime<br/><strong>'+v.overtime+'</strong></div>'+
                                '<div>Late<br/><strong>'+v.late+'</strong></div>'+
                            '</div></div>';

                        i++;
                   });

                   $('.total-drop-task-title').html('<div class="text-center" style="display:flex; justify-content: space-between;">'+
                                '<div>Worked<br/>'+res.data.total.worked+'</div>'+
                                '<div>Overtime<br/>'+res.data.total.overtime+'</div>'+
                                '<div>Late<br/>'+res.data.total.late+'</div></div>')

                   $('#attendance-summary .weeks-list').html(rows);
                }
            });
        }

        // Get attendance summary on page load
        getAttendanceSummary( formatDate('DD-MM-YYYY HH:mm:ss', now) );
